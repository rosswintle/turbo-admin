(() => {
  ...
  var TurboAdminMenuItem = class {
    ...
    static fromItemDefinition(item, element = null, parentTitle = "") {
      const action = item.itemActionType === "url" ? item.itemUrlFunction(element) : item.itemActionInfoFunction(element);
      const menuItem = new TurboAdminMenuItem(item.itemTitleFunction(element), action, parentTitle, item?.noCache);
  ...
  var TurboAdminPlugin = class {
    ...
    async getSearchModeItemDefinitions() {
      ...
      const searchModeItems = this.searchModes.map((searchMode) => {
        ...
        const item = new class_item_definition_default();
        ...
        item.itemActionInfoFunction = () => {
          return {searchMode: searchMode.keyword};
        };
        return item;
      });
  ...
  var ListTableShortcuts = class {
    constructor() {
      ...
      if (document.body.classList.contains("woocommerce-admin-page") && document.body.classList.contains("post-type-shop_order")) {
      ...
      const searchInput = document.querySelector('.search-box input[type="search"], .search-form input[type="search"]');
      if (searchInput) {
  ...
  var TurboAdminContentSearch = class extends class_turbo_admin_plugin_default {
    constructor() {
      ...
      this.searchModes.push(new class_search_mode_default("find", "All post types", this.contentSearch.bind(this), null, false));
    }
    shouldActivate() {
      return true;
    }
  ...
  // src/plugins/class-turbo-admin-woocommerce.js
  var TurboAdminWoocommerce = class extends class_turbo_admin_plugin_default {
    constructor() {
      super("WooCommerce");
      this.searchModes.push(new class_search_mode_default("order", "Orders", this.orderSearch));
      this.searchModes.push(new class_search_mode_default("customer", "Customers", this.customerSearch));
    }
    shouldActivate() {
    ...
    async customerSearch(searchString) {
      ...
      return results.map((result) => {
        ...
        item.url = globalThis.taWp.siteUrl + `/user-edit.php?user_id=${result.id}`;
        item.submenuItems.push(new class_submenu_item_default("Orders", globalThis.taWp.siteUrl + `/edit.php?post_status=all&post_type=shop_order&_customer_user=${result.id}`));
        item.submenuItems.push(new class_submenu_item_default("Profile", globalThis.taWp.siteUrl + `/user-edit.php?user_id=${result.id}`));
        return item;
      });
    }
  };
  var class_turbo_admin_woocommerce_default = TurboAdminWoocommerce;

  // src/plugins/class-turbo-admin-gravity-forms.js
  var TurboAdminGravityForms = class extends class_turbo_admin_plugin_default {
    constructor() {
      super("GravityForms");
      this.searchModes.push(new class_search_mode_default("form", "Gravity Forms", this.formSearch, this.defaultFormsList.bind(this)));
    }
    shouldActivate() {
      return globalThis.contentApi.active && document.getElementById("wp-admin-bar-gravityforms-new-form") !== null;
    }
    activate() {
      super.activate();
    }
    ...
    async formSearch(searchString) {
      ...
      for (let k = 0; k < resultKeys.length; k++) {
        ...
        item.url = globalThis.taWp.siteUrl + `/post.php?post=${result.id}&action=edit`;
        item.submenuItems.push(new class_submenu_item_default("Edit Form", globalThis.taWp.siteUrl + `/admin.php?page=gf_edit_forms&id=${result.id}`));
        item.submenuItems.push(new class_submenu_item_default("Entries", globalThis.taWp.siteUrl + `/admin.php?page=gf_entries&id=${result.id}`));
        item.submenuItems.push(new class_submenu_item_default("Form Settings", globalThis.taWp.siteUrl + `/admin.php?page=gf_edit_forms&view=settings&subview=settings&id=${result.id}`));
        item.submenuItems.push(new class_submenu_item_default("Confirmatons", globalThis.taWp.siteUrl + `/admin.php?page=gf_edit_forms&view=settings&subview=confirmation&id=${result.id}`));
        item.submenuItems.push(new class_submenu_item_default("Notifications", globalThis.taWp.siteUrl + `/admin.php?page=gf_edit_forms&view=settings&subview=notification&id=${result.id}`));
        turboAdminLog(item);
  ...
  var class_turbo_admin_gravity_forms_default = TurboAdminGravityForms;
  ...
  var TurboAdmin = class {
    ...
    async init() {
      ...
      if (window.turboAdminIsExtension()) {
        new class_turbo_admin_woocommerce_default();
        new class_turbo_admin_gravity_forms_default();
      }
    ...
    async addAdditionalMenuItems() {
      ...
      extraItems.forEach((item) => {
        ...
        elements.forEach((element) => {
          const newItem = class_turbo_admin_menu_item_default.fromItemDefinition(item, element, "");
